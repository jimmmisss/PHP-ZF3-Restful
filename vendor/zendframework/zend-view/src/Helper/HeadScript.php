 ...
 * @method HeadScript offsetSetFile($auth, $src, $type = 'text/javascript', $attrs = [])
 ...
 * @method HeadScript offsetSetScript($auth, $src, $type = 'text/javascript', $attrs = [])
    ...
    public function __call($method, $args)
    {
        if (preg_match('/^(?P<action>set|(ap|pre)pend|offsetSet)(?P<mode>File|Script)$/', $method, $matches)) {
            ...
            if ('offsetSet' == $action) {
                ...
                if (1 > count($args)) {
                    throw new Exception\BadMethodCallException(sprintf(
                        'Method "%s" requires at least two arguments, an auth and source',
                        $method
                    ));
                }
            }
